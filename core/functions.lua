...
function CalculateDays(data1, data2)
    local diff = math.abs(data2 - data1)
    return math.floor(diff / 86400)
end

function CalculateCenterPoint(v1, v2, v3, v4)
    local sumX, sumY, sumZ = 0, 0,0  
    local is3D = false
    if v1.z or v2.z or v3.z or v4.z then is3D = true end
    sumX = v1.x + v2.x + v3.x + v4.x
    sumY = v1.y + v2.y + v3.y + v4.y
    if is3D then sumZ = (v1.z or 0) + (v2.z or 0) + (v3.z or 0) + (v4.z or 0) end
    local center = { x = sumX / 4, y = sumY / 4 }
    if is3D then center.z = sumZ / 4 end
    return center
end
