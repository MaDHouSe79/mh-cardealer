Framework, TriggerCallback, OnPlayerLoaded, OnPlayerUnload, OnJobUpdate = nil, nil, nil, nil, nil
...
function HasAccess()
    ...
    if Config.UseAsJob then
        if PlayerData.job.name == Config.JobName then hasAccess = true end
    else
        hasAccess = true
    end
end
...
function Notify(message, type, length)
    if GetResourceState("ox_lib") ~= 'missing' then
        lib.notify({title = "MH Cardealer", description = message, type = type})
    else
        if Config.Framework == "qb" then
            Framework.Functions.Notify({text = "MH Cardealer", caption = message}, type, length)
        end
    end
end
