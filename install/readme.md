...
```lua
['cardealer2'] = { ['label'] = 'Cardealer', ['isManaged'] = false },
```

# Replace Code in
- `qb-garages/html/scripts.js` find the function populateVehicleList
- and replace it with this function below, 
- this is for the garage menu so you can't use the vehicle when you have the it sale.
```js
function populateVehicleList(garageLabel, vehicles) {
    ...
    vehicles.forEach((v) => {
        ...
        if (status === "Add for sale") { // mh-parking 
            driveButton.style.backgroundColor = "#222";
            driveButton.disabled = true;
        }
